...
这里我们可以参考第二节课部署InternLM2-Chat-7B模型去验证下关于huixiangdou问题的原始输出：

![](./image/v2.13.png)

就是需要切换我们上一节课创建的demo虚拟环境，然后直接python解释器去执行`python /root/demo/cli_demo.py`即可。
...
使用RAG检索增强技术通过接入外部知识库，显著的提高模型的准确性以及实时更新性的问题，很好了解决了模型对未曾训练的内容出现幻觉回答，也不需要消耗训练时间继续更新模型。